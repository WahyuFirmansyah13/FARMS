...
<div class="card">
    <div class="card-body">
        ...
        <?php endif; ?>
        <form action="<?= base_url('petani/aktivitas/' . $aktivitas['id_aktivitas']) ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="id_musim" name="id_musim" required>
                    ...
                    <?php foreach ($musim_tanam as $musim) : ?>
                        <option value="<?= $musim['id_musim'] ?>" data-id-tanaman="<?= $musim['id_tanaman'] ?>" <?= (old('id_musim', $aktivitas['id_musim']) == $musim['id_musim']) ? 'selected' : '' ?>><?= esc($musim['nama_tanaman']) ?> (<?= esc($musim['periode_awal']) ?> - <?= esc($musim['periode_akhir']) ?>)</option>
                    <?php endforeach; ?>
                </select>
            </div>
            <input type="hidden" name="id_tanaman" id="id_tanaman" value="<?= old('id_tanaman', $aktivitas['id_tanaman']) ?>">
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="kegiatan" name="kegiatan" value="<?= old('kegiatan', $aktivitas['kegiatan']) ?>" required>
            </div>
            <div class="mb-3">
                <label for="keterangan" class="form-label">Deskripsi</label>
                <textarea class="form-control" id="keterangan" name="keterangan" rows="3"><?= old('keterangan', $aktivitas['keterangan']) ?></textarea>
            </div>
            <div class="mb-3">
                ...
                <?php if ($aktivitas['foto']) : ?>
                    <div class="mt-2">
                        <img src="<?= base_url('uploads/' . $aktivitas['foto']) ?>" alt="Foto Aktivitas" style="max-width: 200px;">
                    </div>
                <?php endif; ?>
            </div>
